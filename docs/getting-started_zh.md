...
| 分类 | 能力清单 |
|------|----------|
| **🤖 智能体核心** | 流式打断不丢工具 · 子智能体后台并行 · 自唤醒（定时/条件触发）· Mission 长任务追踪 · 反幻觉机制 |
...
| **🛠 系统工具** | 文件读写 + Diff Review · 全盘搜索（内置 ripgrep + fd）· 命令行（CMD/PowerShell/bash）· Python 持久环境 · 系统监控 · 软件管理（winget） |
...
| **🔒 安全保护** | 主人密钥认证 · 启动自动锁屏 · 灵魂数据机器绑定加密 · 陌生人识别保持距离 |
| **🤝 协议兼容** | OpenAI / Anthropic / Google / Ollama 多厂商 · Function Calling 统一规范 · **MCP 协议**（外部 MCP Server 即插即用） |
| **🎭 人格 + 扩展** | 基因组 v3 定义（性格/记忆/关系/称呼/说话风格）· Skill 系统（教她新技能）· 元 Skill 机制 |
