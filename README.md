...
> **场景**：你叫她"艾薇"——她记得自己是艾薇。无论换 DeepSeek、Claude、还是 Kimi 底座，**她依然是艾薇**，性格、说话风格、记忆都不变。

底座只是引擎，**她才是车主**。基因组 v3 系统让她真的有"自己"——而不是 Claude/GPT 套壳。

---

### 🌊 你能随时打断她，工具结果不丢

> **场景**：让她"用 Python 写个数据分析"——她搜了一半 pandas 文档、写了一半代码，你忽然想起："等等，用 polars 更快"。
> **她的反应**：保留刚搜到的文档，把代码切到 polars 上继续——**不重头来过**。

主对话流式输出/工具调用过程中你随时插话——她吸收新消息继续推理，已执行的工具结果保留。

---

### 🤖 派她做事，你继续和她聊

> **场景**：让她做三件事——"持续监控 GitHub 仓库 / 研究 LangGraph 源码 / 写每周总结"。
> **她的反应**：派出 **3 个分身后台并行跑**，主对话不阻塞，你和她继续聊别的。做完一个，她主动回来汇报。

不是单线程串行——是真正的多线程后台。

---

### 🎨 IDE 三栏 + Diff Review，比 Cursor 多了"撤销时间线"

> **场景**：让她重构整个项目——文件树左侧、Monaco 编辑器中间、对话右侧。每一处改动 Diff 高亮，**你点 Accept/Reject 逐块审核**。
> **额外的安全网**：即使全接受了，**48 小时内任何改动都能逐条回滚**——Cursor 没有的撤销时间线。

最多保留 200 条改动历史，48 小时内任意时间点回退。
...
> **场景**：
> - "帮我把这张照片去背景" → 她真的打开 **Photoshop**、用魔棒/钢笔工具、保存
> - "帮我给妈发个微信说晚饭不回家" → 她真的打开微信、找联系人、输入文字、发送
> - "把这 10 份 PDF 的重点整理到 Excel" → 她真的填进 Excel

不只浏览器自动化（这个大家都有）——**真实键鼠操作任何 GUI 软件**。
...
> **场景**：
> - 早上你在网页跟她讨论方案
> - 中午出门，**微信发消息给她**——她接着早上的话题继续
> - 晚上回家，**桌面悬浮窗双击**找她——她记得你这一整天
...
> **场景**：万一电脑被偷了——别人拷走整个 Aivy 文件夹想看你和她聊了什么？
> **结果**：基因/记忆/画像**加密绑定本机**——别的电脑打不开。

- 启动自动忘记上次认证状态——必须重新输入主人密钥
- 不认识你的人来接近她，她**保持距离不暴露真实自己**
- 检测到不对劲，她会自己锁屏
...
> **场景**：
> - "每天早 8 点告诉我天气 + GitHub 仓库 Star 数变化"
> - "收到新邮件就告诉我"
> - "追踪我这个创业项目的进度，每周给我一份总结"（**Mission 系统**，跨天跨周追踪）

**自唤醒** + **Mission 长任务追踪**——她不是被动等你说话的工具。

---

### 🧠 三层记忆 + 离线归档 + 元 Skill 自学

> **场景 1**：两周前你提过想去日本——今天她帮你查机票时**主动问"还是去东京吗"**。
> **场景 2**：你下线了，但她在跑后台任务——任务做完她**先归档到记忆**，等你上线主动汇报。
> **场景 3**：你教她一个工作流（"先 grep 找文件，再 python 处理，最后生成报告"）——她**自动归纳成可复用的 Skill**，下次遇到类似任务直接复用。

- **核心记忆**：永远不忘的事实
- **长期记忆**：向量检索的对话历史（5000+ 条仍能精准查到）
- **元 Skill**：从对话中自动归纳可复用的工作流
- **异常关闭补录**：电脑突然崩溃，重启后她自动补录关闭前的对话

---

### ⚡ 适配 DeepSeek V4 1M 上下文 + 30+ 工具 + MCP

- DeepSeek V4 Pro 的 **1M 长上下文** —— 整本书塞进去聊都行
- v1.7.2 实测 **99.47% cache 命中**，首字延迟 **<1 秒**
- **30+ 内置工具**：文件/浏览器/桌面/Python/网搜/PDF/视觉/系统监控 等
- **MCP 协议支持**：可接外部 MCP Server（GitHub/Slack/邮件/数据库等）—— 不会的事可以学
