...
> 修复了一个会导致对话中断的安装位置问题；语音转到后台后，可以像平时一样发图片和文件。

| 更新 | 说明 |
|---|---|
| 🗂️ **安装位置修复** | 修复安装在 `C:\Program Files`（安装时选"所有人"）时，她回复到一半就报错中断的问题。用户数据现统一存放在个人目录，**装在任何位置都能正常使用**；更换安装方式重装，激活信息与聊天记录也不会丢失 |
| 📎 **语音后台可发附件** | 语音转到后台后，聊天输入框可以像平时一样发送图片、文档和音视频，不再只能发纯文字 |
| 🔀 **转文字模式按钮** | 语音界面右上角新增：回聊天界面继续打字，语音仍在后台运行、回复照常念出。接通后才可点击，避免连接中误触；右上角 ✕ 现明确标注为「结束语音」 |
| 🎧 **耳机中断修复** | 修复戴耳机时麦克风短暂中断（蓝牙丢包、耳机切换模式），第二次中断会被弹出语音界面、且该轮说话内容丢失的问题 |
| 📄 **大文件按需读取** | 发送较大文本文件时不再把整篇塞进对话，改为按需读取——长文件不再一次占满上下文，小文件仍直接可见 |
| 🗑️ **卸载提示** | 卸载时告知聊天记录与激活信息的保存位置，并可一键打开该文件夹（卸载不删除这些数据，重装即可继续使用） |

**建议所有用户升级到本版本。** 完整说明见 [Release Notes v2.2.2](https://github.com/Bo1202/Aivy-OS/releases/tag/v2.2.2)。
