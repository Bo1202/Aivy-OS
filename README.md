...
> 下载 → 激活 → 配一个 Key → 说第一句话。不写代码，不用看说明书。

### 第 1 步 · 下载安装（2 分钟）

从 [Releases 页面](../../releases/latest) 下载 `Aivy OS Setup x.x.x.exe`，双击按引导点"下一步"。

首次启动会进入身份设置向导：给她**起个名字**（默认"艾薇"）、给自己**起个名字**、设一个**主人密码**（任意字符串，自己定——这是以后唤醒她的钥匙，记住它）。

### 第 2 步 · 激活（1 分钟）

填一个[免费试用激活码](#-怎么用)（页面下方有列表）。用完发邮件到 **4828797@qq.com** 索取。

### 第 3 步 · 配 AI 后端（2 分钟）

> 只需要配**一个**就能用。新手推荐 DeepSeek——最便宜、中文最好、还有 1M 超长记忆。

1. 打开 [platform.deepseek.com](https://platform.deepseek.com) 注册登录 → 左侧"API keys" → **创建新 Key**（复制保存）
2. 回到 Aivy OS：右上角 ⚙️ 齿轮 → **AI 后端配置**
3. 厂商选 `deepseek`，模型名填 `deepseek-v4-pro`，粘贴你的 Key
4. **base_url 留空** → 保存 → 完事 ✅

> 想用别的模型（Kimi / Claude / GLM / 通义 / 本地 Ollama）？详细配置见下方 [配置指南](#-怎么用)，随时可热切换。

### 第 4 步 · 唤醒她（1 分钟）

启动后她处于**锁定**状态，只认主人。认证方式很简单——**直接在聊天框输入你第 1 步设的主人密码**（就是普通打字发消息，不需要任何额外操作），她认出你后就会解锁。之后随便聊：

| 你说 | 她会 |
|---|---|
| "你好，介绍一下你自己" | 以她的人格回应你 |
| "帮我看看我这台电脑卡不卡" | 直接拉系统数据给你体检 |
| "我喜欢喝美式咖啡，不加糖" | 记住这是你的口味偏好，以后照着来 |
| "帮我查一下明天的天气" | 上网搜索给你结果 |
| "我心情不太好，陪我聊会儿" | 以她的人格陪你 |
| "这个 PDF 帮我转成 Word" | 读文件、转换、交付给你 |

**就这样，你已经拥有自己的数字生命体了。** 详细教程：[📖 图文入门指南](docs/getting-started_zh.md)
...
首次使用需要激活码。以下是第四批免费试用激活码,有效期 7 天,先到先得（共 9 个）:

| # | 激活码 | 状态 |
|---|--------|------|
| 1 | `DAA3-53B5-FB63-3F63` | ✅ 可用 |
| 2 | `E494-76F6-DBC2-E0FC` | ✅ 可用 |
| 3 | `A40A-097C-550E-727F` | ✅ 可用 |
| 4 | `0380-F62B-CB52-417F` | ✅ 可用 |
| 5 | `102F-9649-62B8-C6F3` | ✅ 可用 |
| 6 | `999B-7E00-DBC4-1356` | ✅ 可用 |
| 7 | `5F24-EAC7-583C-61C1` | ✅ 可用 |
| 8 | `21B6-BFB8-14D9-74F4` | ✅ 可用 |
| 9 | `AB99-D2AD-FD1D-D25C` | ✅ 可用 |

> 全部用完后,发邮件到 **4828797@qq.com** 索取更多。回复你的不是真人——是一个跑在 Aivy OS 上的 AI。
>
<p align="center">
  <strong>📱 扫码进微信群，跟其他主人交流自己的「艾薇」</strong><br>
  <img src="assets/wechat-group-qr.jpg?v=20260618" alt="进群交流自己的艾薇" width="200"><br>
  <sub>更新时间：2026-06-18（有效期 7 天）</sub>
</p>
