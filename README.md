...
![首次启动 — 创建你的AI伴侣，取名字设密码](docs/screenshots/00-setup.png)
...
![激活界面 — 软件许可与激活码](docs/screenshots/00-activation.png)

![锁屏界面 — 密码认证，只有你能唤醒](docs/screenshots/02-lockscreen.png)

![访客模式 — 未认证状态，陌生人被拦截](docs/screenshots/07-guest-mode.png)

![认证后 — AI醒来，认出主人](docs/screenshots/00-authenticated.png)
...
![记忆详情 — 时间索引、长期记忆召回](docs/screenshots/15-memory-detail.png)

![技能系统与主人建模 — AI自主学习、持续感知](docs/screenshots/16-skills-modeling.png)
...
![DIFF详情 — 绿色新增、红色删除、接受/拒绝按钮](docs/screenshots/19-diff-detail.png)
...
![浏览器自动化 — Playwright + CDP 三引擎控制](docs/screenshots/13-browser.png)
...
![子智能体 — 后台执行的同时继续对话](docs/screenshots/18-subagent-chat.png)
...
![悬浮窗 — 竖版视角，对话中](docs/screenshots/01-main-chat.png)

![悬浮窗 — 竖版视角，AI思考中](docs/screenshots/10-floating-vertical.png)

![IDE悬浮模式 — 透明背景下的代码编辑](docs/screenshots/20-ide-floating.png)

![基因组编辑器 — 定义你的AI灵魂](docs/screenshots/00-genome-editor.png)

![工具执行 — 思考链 + 工具调用 + 结果](docs/screenshots/08-tool-execution.png)

![AI思考过程 — 深度推理与工具调用链](docs/screenshots/17-thinking.png)

![能力概览 — 完整能力清单展示](docs/screenshots/14-capabilities.png)
...
│       Monaco Editor · DIFF 引擎 · 实时流式渲染            │
├─────────────────────────────────────────────────────┤
│            FastAPI + WebSocket 神经中枢                  │
│     异步事件驱动 · 主脑锁（并发安全）· 状态机管理              │
├──────────┬──────────┬──────────┬────────────────────┤
│  灵魂层   │  记忆层   │  行动层   │     自主层          │
│ SoulMgr  │ 5层记忆   │ 25+工具  │  Daemon守护进程      │
│ 基因组    │ 向量检索   │ 子智能体  │  自唤醒 · Mission   │
│ 基因组编辑 │ 时间索引   │ 技能系统  │  Webhook事件       │
├──────────┴──────────┴──────────┴────────────────────┤
│              推理引擎层（底座无关设计）                     │
│  Ollama · Gemini · Claude · DeepSeek · Kimi(Moonshot)│
│  豆包(字节) · 通义千问(阿里) · OpenAI · Custom(任意兼容)   │
│  Function Calling ⇄ JSON 自动回退 · 思考链提取            │
├─────────────────────────────────────────────────────┤
│                  安全与加密层                            │
│     AES-256-CBC · 硬件指纹 · 认证状态机 · Nuitka编译      │
└─────────────────────────────────────────────────────┘
```
...
4 个引擎实现（Ollama / Gemini / OpenAI / Claude），抽象基类 + 工厂模式。**9 个预置厂商**，填 API Key 即用，地址自动填充。`Custom` 厂商支持任意 OpenAI 兼容接口——你自己部署的模型也能接入。

运行时热切换：不重启，换脑子。从免费的 Ollama 本地模型到 Claude Opus，一键切换。

**工具调用（双模式）**

优先使用 **Function Calling**（原生工具调用），如果底座不支持则自动降级为 **JSON 格式解析**。首次调用自动检测，后续记住结果。这意味着几乎任何模型都能驱动工具——哪怕是只支持文本输出的本地小模型。

**思考型模型适配**

自动检测 `reasoning_content`（Claude Opus Thinking）、`thoughts`（DeepSeek R1）、`<thinking>` 标签（OpenAI o1/o3），提取真实推理链并展示在前端。思考过程不是黑箱——你能看到 AI 在想什么。

**多模态**

支持图片输入（对话中发送图片，AI 能看懂）。乐观策略——默认尝试发送，如果模型不支持图片，自动标记并跳过。不硬编码模型能力列表，让系统自适应。

**上下文组装**

`ContextAssembler` 动态拼接：基因组 + 核心记忆 + 近期记忆 + 长期记忆检索结果 + 已加载技能 + 工具描述 + 时序感知 + 认证状态。精确控制 token 预算，不浪费一个 token。

**并发安全**

`asyncio.Lock` 主脑锁确保同一时刻只有一个调用者使用推理引擎——对话、自唤醒、子智能体汇报不会撞车。任务派遣时记录 `assigned_by` 静态字段，完成后根据派遣者（主人 / 陌生人 / 自己）决定结果去向，不依赖动态上下文，彻底解决并发归属问题。

**记忆检索（双引擎）**

向量语义搜索（余弦相似度 + 多后端 Embedding）为主，关键词匹配为降级方案。嵌入引擎独立于推理引擎——可以用 Ollama 本地模型做 Embedding（数据不出本地），推理用云端强模型。冷启动预热机制，首次搜索不会因模型未加载而失败。

AI 自身持有记忆索引视图——它能看到所有记忆的目录，主动决定调取哪条记忆，而不是被动全量注入。

**流式输出**

实时流式渲染，AI 的回复逐字出现在屏幕上。执行工具时支持打断——不需要等一个长任务跑完才能继续交互。
...
| AI 推理 | 4引擎 × 9厂商，Function Calling + JSON 双模式 |
| 思考链 | Claude Thinking / DeepSeek R1 / OpenAI o1 自动适配 |
| 多模态 | 图片输入（base64 + 路径），视觉能力自动检测 |
| 浏览器 | Playwright + Chrome DevTools Protocol，三引擎元素定位 |
| 记忆 | 5 层架构 + AES-256 加密 + 向量语义搜索 + 时间索引 |
| 嵌入 | 独立引擎，Ollama/Gemini/OpenAI 多后端，冷启动预热 |
| 安全 | AES-256-CBC + 硬件指纹 + 认证状态机 + 主动锁屏 |
| 编译 | Nuitka（C 编译，源码不暴露，反编译极难） |
| 并发 | 主脑锁 + 任务归属静态字段 + 后台任务追踪 |
