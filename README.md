...
由于 Aivy OS 当前未签名,Win11 安全机制可能影响启动速度（**Win10 不受影响**）。

**先验证真因** — 打开**飞行模式**后启动 Aivy OS:
- ✅ **秒开** = SmartScreen 联网做云端 reputation check 是真因 → **方案 A**
- ❌ **仍慢** = Defender 实时扫描 / Smart App Control 是真因 → **方案 B**

**方案 A**（SmartScreen 真因）：设置 → 隐私与安全 → Windows 安全中心 → 应用和浏览器控制 → 关闭"基于声誉的保护"中的"检查应用和文件"

**方案 B**（Defender 真因）：Windows 安全中心 → 病毒和威胁防护 → 管理设置 → 排除项 → 添加文件夹 `C:\Users\<你的用户名>\AppData\Local\Programs\Aivy OS\`

**两个都试也行**（互不冲突）。**长期解决**：我们正在申请 Windows 代码签名证书,签完后所有 Win10/Win11 都秒开。
