...
[![Version](https://img.shields.io/badge/version-1.7.2-blue.svg)](https://github.com/Bo1202/Aivy-OS/releases)
...
## 🆕 v1.7.2 更新（2026-04-25 · 适配 DeepSeek V4 1M 上下文）

主要面向用 DeepSeek V4 的用户做了 cache 适配，对话速度有比较明显的改善：

| 场景 | v1.7.1 | v1.7.2 |
|---|---|---|
| DeepSeek V4 首字延迟 | ~4 秒 | <1 秒 |
| DeepSeek V4 Cache 命中率 | 0% | 99.47% |
| 工具链中插话/打断恢复 | 不支持 | ✅ 无感继续 |

其他改进：记忆归档底层修复 / 浏览器工具稳定性 / UI 拖拽体验 / 一些小修复。完整清单见 [Release Notes](https://github.com/Bo1202/Aivy-OS/releases/tag/v1.7.2)。

---

## 🦞 和 OpenClaw、Hermes Agent 的对比

Aivy OS 和最近热门的 [OpenClaw](https://github.com/openclaw)（小龙虾）、[Hermes Agent](https://github.com/NousResearch/hermes-agent)（爱马仕，6.6 万星）都是**本地自主 AI Agent 框架**——给 AI 一台电脑让它自己操作。

定位区别：OpenClaw / Hermes 偏**通用 Agent 工具**；Aivy OS 偏**专属 AI 伴侣**——有名字、有性格、有记忆的"她"。

### 完整功能对比

| 维度 | OpenClaw 🦞 | Hermes Agent 👜 | **Aivy OS** |
|---|---|---|---|
| **核心定位** | 通用 Agent 工具 | 持续学习智能体 | 专属 AI 伴侣（本地化的"她"）|
| **License** | 开源 | 开源 | 闭源 + 激活码 |
| **本地部署** | ✅ | ✅ | ✅（Windows 一键安装包）|
| **多模型支持** | ✅ | ✅ | ✅（v1.7.2 适配 DeepSeek V4 1M 上下文）|
| **MCP 协议** | ✅ | ✅ | ⚠️ 可选 |
| **多通道接入** | 飞书/微信/钉钉/Slack/Teams | Telegram/Discord/Slack/WhatsApp/微信 | **微信/网页/邮件/桌面悬浮窗（同一个她）** |
| **文件操作** | ✅ | ✅ | ✅ + **Diff Review 逐块审核 Accept/Reject** |
| **浏览器自动化** | ✅ | ✅ | ✅（Playwright + 跨标签 + iframe）|
| **命令行执行** | ✅ | ✅（Docker/SSH/Modal）| ✅（CMD/PowerShell/bash）|
| **桌面控制（任意软件）** | ⚠️ 未强调 | ⚠️ 未强调 | ✅ **PS / 微信 / Office / 游戏 等任意 GUI 软件** |
| **IDE 三栏开发模式** | ❌ | ❌ | ✅ **文件树 + Monaco 编辑器 + 对话** |
| **撤销时间线** | ❌ | ❌ | ✅ **任何改动可逐条回滚（max 200 条 / 48h）** |
| **流式中断恢复** | ❌（打断丢工具结果）| ❌（同）| ✅ **AI 说话中可插嘴，工具结果不丢** |
| **多智能体并行** | ⚠️ 有概念但偏单线 | ⚠️ 同 | ✅ **N 个分身后台并行互不干扰** |
| **跨会话记忆** | ⚠️ 基于 LLM 摘要 | ✅ FTS5 + LLM 摘要 | ✅ **三层架构（核心/长期/短期）+ 用户画像** |
| **自主技能创建** | ⚠️ 用户写 Skill | ✅ **自动生成 Skill 文件**（Hermes 王牌）| ⚠️ 通过基因组 + Wiki 引导（需用户参与）|
| **Skill / 插件商店** | ✅ | ❌ | ⚠️ 自带 Skills 但无在线商店 |
| **AI 人格定制** | ❌ 通用 Agent | ❌ 通用 Agent | ✅ **基因组定义性格/记忆/关系/称呼** |
| **认证保护 + 启动锁** | ⚠️ 聊天 ID 识别 | ⚠️ 同 | ✅ **主人密钥 + 启动自动忘记上次认证** |
| **数据加密** | ⚠️ 本地明文 | ⚠️ 本地明文 | ✅ **机器绑定加密**（拷走文件夹也打不开）|
| **自唤醒** | ⚠️ 定时任务 | ⚠️ 同 | ✅ **定时/条件触发 + AI 自主决定** |
| **长期任务追踪** | ❌ | ❌ | ✅ **Mission 系统（跨天跨周）**|
| **离线归档 + 上线汇报** | ❌ | ⚠️ 跨会话记忆 | ✅ **异常关闭补录 + 主动汇报**|
| **DeepSeek V4 cache 适配** | ⚠️ 通用实现 | ⚠️ 同 | ✅ **v1.7.2 命中 99.47%**|
| **GitHub Stars** | 中 | **6.6 万** | 较少（个人项目）|

### Aivy OS 独有的能力（OpenClaw / Hermes 没有的）

如果你看重下面这些场景，可以考虑试一下：

1. **🤝 一个固定的"她"** — OpenClaw / Hermes 是无人格 Agent；Aivy 有名字、性格、记忆，永远是同一个她
2. **🎨 IDE 三栏 + Diff Review** — 不是"AI 写完你复制粘贴"，是**逐块 Accept/Reject**，像 Cursor 一样
3. **↩️ 撤销时间线** — 任何代码/配置改动 48 小时内可逐条回滚（OpenClaw / Hermes 没有这个机制）
4. **🌊 流式中断不丢工具** — AI 说话/调工具中你能插嘴，已执行的结果保留（v1.7.2 强化）
5. **🤖 子智能体真后台并行** — 派分身研究/写代码，主对话不阻塞
6. **🖥 桌面任意 GUI 控制** — 不只浏览器，PS/Office/游戏 全能控
7. **🎯 Mission 长任务追踪** — 跨天跨周追踪一个目标
8. **🔒 机器绑定加密** — 灵魂数据拷走文件夹也打不开
9. **📞 多通道同一个她** — 微信/网页/邮件/桌面悬浮窗，状态/记忆/认证统一
10. **⚡ DeepSeek V4 1M 上下文 + 99.47% cache 命中** — v1.7.2 专项适配

### Hermes Agent 比 Aivy OS 强的地方（诚实承认）

- **自主生成 Skill 文件** — Hermes 完成任务后会自动判断"这个流程值得记下来吗"，自动生成可复用的 Skill。Aivy 目前需要用户引导
- **MCP 默认开启** — Hermes / OpenClaw 默认接 MCP，Aivy 是可选项
- **GitHub 6.6 万星** — Hermes 社区生态远比 Aivy 大
- **跨终端运行** — Hermes 支持 Docker / SSH / serverless Modal 多种 runtime；Aivy 只在 Windows 桌面

### 总结一句

> **想要一个无人格的强力 Agent？** OpenClaw / Hermes 都很优秀。
> **想要一个永远只属于你这一台电脑、有人格、有记忆的"她"？** 试试 Aivy OS。
...
| 你想要 | 选这个 | 模型名 | 为什么 |
|---|---|---|---|
| **首选 / 综合最佳** | DeepSeek V4 Pro | `deepseek-v4-pro` | **1M 长上下文** + cache 99.47% 命中（v1.7.2 适配）+ 极便宜 + 中文好 |
...
| 模型 | Function Calling | thinking 模式 | v1.7.2 cache 优化 |
|---|---|---|---|
| **DeepSeek V4 Pro** | ✅ 稳定 | ✅（v1.7.2 已修边界）| ✅ 99.47% |
...
输入主人密码（如 `aw-1202`）认证 → 她"认出你"显示主人欢迎语 → 之后随便聊。
