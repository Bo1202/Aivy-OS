...
## 🆕 v1.7.2 重点更新（2026-04-25）

- ⚡ **DeepSeek V4 用户对话首字延迟 4 秒 → <1 秒**（cache 命中率 0% → 99.47%）
- 🧠 **记忆系统大修**：归档机制底层架构修复，长期记忆质量随使用时长稳步提升
- 🛡️ **数据安全增强**：用户画像存储底层重构，消除"整文件被破坏"风险
- 🌐 **浏览器工具更稳定**：彻底修复某些场景下的 asyncio 冲突
- 🔍 **内置 ripgrep + fd 搜索引擎**：全盘搜索性能 10-100 倍
- 🎨 **UI 拖拽 / 撤销时间线 / IDE 三栏 全面优化**

完整修复清单见 [Release Notes](https://github.com/Bo1202/Aivy-OS/releases/tag/v1.7.2)。
...
| 服务 | 优势 | 速度 | 工具调用 | 费用 |
|------|------|------|---------|------|
| **DeepSeek V4** | v1.7.2 cache 99.47% 命中,首字延迟 <1 秒 | ⚡⚡⚡ | ✅ 优秀 | 极便宜（输入缓存 0.5 元/百万 token）|
| **Kimi K2.6** | 长上下文（128K+）,工具调用稳定 | ⚡⚡ | ✅ 优秀 | 中等 |
| **智谱 GLM-4.5** | 中文优化好,thinking 模式强 | ⚡⚡ | ✅ 优秀 | 中等 |
| **Claude Sonnet 4** | 推理质量最高,工具调用最稳 | ⚡⚡ | ✅ 极优秀 | 较贵 |
| **OpenAI GPT-5** | 通用能力强 | ⚡⚡ | ✅ 优秀 | 较贵 |
| **Gemini 2.5** | 多模态视觉强 | ⚡⚡ | ✅ 良好 | 中等 |
| **Ollama 本地** | 完全本地、零费用、隐私最强 | ⚡（取决于硬件）| ⚠️ 取决于模型 | 免费 |

#### 🔑 各厂商 API Key 获取

| 厂商 | Key 申请链接 | 配置示例 |
|------|------------|---------|
| **DeepSeek** | [platform.deepseek.com](https://platform.deepseek.com) → API Keys | 选 `deepseek` 厂商 + `deepseek-v4-pro` 模型名 + 粘贴 `sk-xxx` |
| **Kimi (月之暗面)** | [platform.moonshot.cn](https://platform.moonshot.cn) → API Keys | 选 `kimi` + `kimi-k2.6` + 粘贴 Key |
| **智谱** | [open.bigmodel.cn](https://open.bigmodel.cn) → 个人中心 → API keys | 选 `glm` + `glm-4.5` + 粘贴 Key |
| **Claude** | [console.anthropic.com](https://console.anthropic.com) → API Keys | 选 `claude` + `claude-sonnet-4-5` + 粘贴 `sk-ant-xxx` |
| **OpenAI** | [platform.openai.com/api-keys](https://platform.openai.com/api-keys) | 选 `openai` + `gpt-4o` + 粘贴 `sk-xxx` |
| **Gemini** | [aistudio.google.com/apikey](https://aistudio.google.com/apikey) | 选 `gemini` + `gemini-2.0-flash` + 粘贴 Key |
| **阿里通义千问** | [bailian.console.aliyun.com](https://bailian.console.aliyun.com) | 选 `custom` + `qwen-plus` + base_url `https://dashscope.aliyuncs.com/compatible-mode/v1` + Key |
| **Ollama 本地** | 安装 [Ollama](https://ollama.com) + `ollama pull qwen3` | 选 `ollama` + `qwen3` + 留空 Key（本地不需要）|

#### 💡 配置小技巧

- **首次试用** 推荐 DeepSeek V4——最便宜，v1.7.2 后速度最快
- **多个 Key 并存** 设置面板可以同时填多个厂商，主菜单一键切换
- **代理用户** 如果走代理，可以填自定义 `base_url`（OpenAI 兼容模式都支持）
- **本地隐私优先** 用 Ollama + 大模型（如 qwen3:32b、deepseek-coder-v2 等），完全离线
- **工具调用要稳** 选 Function Calling 支持的模型（DeepSeek/Kimi/GLM/Claude/GPT 都支持）

### 4. 开始使用

她现在是你的了。聊天、让她做事、给她任务、和她一起度过日常。

📖 **更详细的配置教程**：[docs/getting-started_zh.md](docs/getting-started_zh.md)（含截图引导）
