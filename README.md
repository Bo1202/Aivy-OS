...
## 🦞 和 OpenClaw、Hermes Agent 的不同

Aivy OS 和最近的 [OpenClaw](https://github.com/openclaw)（小龙虾）、[Hermes Agent](https://github.com/NousResearch/hermes-agent)（爱马仕）都是**本地自主 AI Agent 框架**——给 AI 一台电脑让它自己操作。

它们更像"通用 Agent 工具"。Aivy OS 的定位偏**专属 AI 伴侣**——同一个 AI，有名字、有记忆、跨通道（微信/网页/邮件/桌面）一直陪着你。几个能让你考虑试一下的差别：

### 🌊 流式对话支持随时打断

主对话流式输出过程中，你可以随时插话或补充——AI 会**吸收新消息继续推理**，已执行的工具结果不丢失，不需要重新开始。

### 🤖 子智能体后台并行

派她做研究/写代码/监控时，**主对话不会被阻塞**——分身在后台跑，做完自动汇报。可以同时跑多个分身互不干扰。

### 🔒 灵魂数据本机加密

基因、记忆、画像加密绑定本机——别人拷走文件夹也打不开。启动自动锁定，必须重新认证主人密码。
